...
type TDeckState = Record<GAME_MODES, Array<OperatorCard>>
...
export const [DeckProvider, useDeck] = constate(() => {
  ...
  const updateDeck = useCallback(
    (mode: GAME_MODES, state: Array<OperatorCard>) => {
      setDeckState((prev) => ({
        ...prev,
        [mode]: state
      }))
  ...
  const refreshDeck = useCallback(
    (mode: GAME_MODES) => {
      ...
      //   return
      // }
      if (mode === GAME_MODES.DRINKS) {
        setDeckState((prev) => ({
          ...prev,
          [mode]: getDeckPoolDrinks(mathOperators)
        }))
        return
      }
    },
